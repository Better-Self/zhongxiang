...
<div class="page-container">
    <div class="text-c">
        ...
        <input type="text" name="" id="" placeholder=" 友情链接名称" style="width:250px" class="input-text">
        <button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜友情链接 </button>
    </div>
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" data-title="添加友情链接" data-href="/admin/friend/create" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加友情链接</a></span> <span class="r">共有数据：<strong>{{$count}}</strong> 条</span> </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <tr class="text-c">
                ...
                <td>{{$value->id}}</td>
                <td class="text-l"><u style="cursor:pointer" class="text-primary">{{$value->url}}</u></td>
                <td><a title="{{$value->name}}">{{str_limit($value->name,9,"...")}}</a></td>
                @if($value->type==0)
                    <td class="td-status"><span class="label label-error radius">未使用</span></td>
                @else
                    <td class="td-status"><span class="label label-success radius">使用中</span></td>
                @endif
                 <td class="f-14 td-manage"> <a style="text-decoration:none" class="ml-5" onClick="article_edit('友情链接编辑','/admin/friend/{{$value->id}}/edit')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="article_del(this,{{$value->id}})" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
            </tr>
            @endforeach
            </tbody>
        </table>
