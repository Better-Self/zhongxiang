<!--_meta 作为公共模版分离出去-->
<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
<meta name="renderer" content="webkit|ie-comp|ie-stand">
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta name="viewport" content="width=device-width,initial-scale=1,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no" />
<meta http-equiv="Cache-Control" content="no-siteapp" />
<link rel="Bookmark" href="/favicon.ico" >
<link rel="Shortcut Icon" href="/favicon.ico" />
<!--[if lt IE 9]>
<script type="text/javascript" src="/luntan/lib/html5shiv.js"></script>
<script type="text/javascript" src="/luntan/lib/respond.min.js"></script>
<![endif]-->
<link rel="stylesheet" type="text/css" href="/luntan/static/h-ui/css/H-ui.min.css" />
<link rel="stylesheet" type="text/css" href="/luntan/static/h-ui.admin/css/H-ui.admin.css" />
<link rel="stylesheet" type="text/css" href="/luntan/lib/Hui-iconfont/1.0.8/iconfont.css" />
<link rel="stylesheet" type="text/css" href="/luntan/static/h-ui.admin/skin/default/skin.css" id="skin" />
<link rel="stylesheet" type="text/css" href="/luntan/static/h-ui.admin/css/style.css" />
<script src="/admins/bs/js/jquery.min.js"></script>
<script type="text/javascript">
$(function()
{
	$('input').click(function()
	{
		var level=$(this).attr('level');
	

		if(level==1){
			var str='_';
			var inputs=$('input[value*='+str+']');
			$(this).prop('checked')?inputs.prop('checked',true):inputs.prop('checked',false);
		
		}
		else if(level==2){
			var id=$(this).attr('id');
			
			var inputs=$('input[pid='+id+']');
			$(this).prop('checked')?inputs.prop('checked',true):inputs.prop('checked',false);
		
		}
		else if(level==3){
		
			//if($(this).attr('checked')){

				var pid=$(this).attr('pid');
				
				$('input[id='+pid+']').prop('checked',true);

				var ppid=$('input[id='+pid+']').attr('pid');
				
				$('input[id='+ppid+']').prop('checked',true);

			//}
		
		
		}
		
	});
});
</script>
<title>网站角色 - 管理员管理</title>

</head>
<body>
<article class="page-container">
	<form action="" method="post" id="form-category-add" class="form form-horizontal" >
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>角色名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				{{$rolename->name}}
			</div>
		</div>
	
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">网站角色：</label>
			<div class="formControls col-xs-8 col-sm-9">
				@foreach($data as $value)
				<p style="@if($value->level==3)float:left;margin-right: -40px; @else clear:both;  @endif" >


						<label name="names" >

							{{str_repeat("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",$value->level)}}

							<input id="{{$value->id}}" type="checkbox" name="access[]" value="{{$value->id}}_{{$value->level}}" pid="{{$value->pid}}" level="{{$value->level}}" @if($value->access==1)
							checked="checked"  @endif	 >

							@if($value->level==1)
							<span style="font-weight: bold;">【项目】</span>
							@elseif($value->level==2)
							<span style="font-weight: bold;">【模块】</span>
							@else
							<span style="font-weight: bold;"></span>
							@endif
							{{$value->title}}</label>


				</p>
				@endforeach

				
				<input type="hidden" name="rid" value="{{$rid}}"> 
			</div>
		</div>
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
				<button type="button" onclick="add()" class="btn btn-success radius" id="admin-role-save" name="admin-role-save"><i class="icon-ok"></i> 确定</button>
			</div>
		</div>
	</form>
</article>




</body>
</html>

<script type="text/javascript">

		//添加的处理操作
	function  add(){
			
			//表单序列化
			str=$("#form-category-add").serialize();
			
			
			$.post('/admin/roles/1/rolelist',{'str':str,'_token':'{{csrf_token()}}'},function(data){

				if(data){
					
					var index = parent.layer.getFrameIndex(window.name);
					parent.$('.btn-refresh').click();
					parent.layer.close(index);
					window.location.href = "/admin/role";
				}else{
					
				}

		});
			
	}





</script>